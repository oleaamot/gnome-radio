...
void
gtk_radio_player_main(gchar *streamuri, gchar *name)
{
	...
	/* } */
	/* else { */
	/*   g_message ("Specify the media file name..<gtk_radio-playerplayer filename>"); */
	/*   return 0; */
	/* } */

	gtk_radio->player_loop = g_main_loop_new (NULL, FALSE);
	g_main_loop_run (gtk_radio->player_loop);
	gst_deinit();
...
void
gtk_radio_player_new (GstPlayer * player, const gchar * next_uri)
{
	gst_player_set_uri(player, next_uri);
}

void
gtk_radio_player_stop (GstPlayer *player)
{
	if (player != NULL) {
...
void
gtk_radio_player_pause (GstPlayer *player)
{
