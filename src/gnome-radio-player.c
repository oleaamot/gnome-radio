...
#include "gnome-radio-player-kb.h"
...
void
gnome_radio_player_main(gchar *streamuri, gchar *name)
{
	...
	/* } */
	/* else { */
	/*   g_message ("Specify the media file name..<gnome_radio-playerplayer filename>"); */
	/*   return 0; */
	/* } */

	gnome_radio->player_loop = g_main_loop_new (NULL, FALSE);
	g_main_loop_run (gnome_radio->player_loop);
	gst_deinit();
	gnome_radio->player_status = GNOME_RADIO_PLAYER_TRUE;
	exit (0);
}
...
void
gnome_radio_player_new (GstPlayer * player, const gchar * next_uri)
{
	gst_player_set_uri(player, next_uri);
}

void
gnome_radio_player_stop (GstPlayer *player)
{
	if (player != NULL) {
...
void
gnome_radio_player_pause (GstPlayer *player)
{
