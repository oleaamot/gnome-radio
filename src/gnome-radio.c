...
#include "gnome-radio-location.h"
#include "gnome-radio-markers.h"
...
GList *gnome_radio_programs;
GList *gnome_radio_stations;
GList *gnome_radio_streams;
...
extern struct GNOMERadioMedia *media;
...
gchar *
str_channels (GNOMERadioChannels type) {
  gchar *channels;
  if (type == GNOME_RADIO_CHANNELS_MONO) {
    channels = g_strdup("Mono");
  }
  if (type == GNOME_RADIO_CHANNELS_STEREO) {
    channels = g_strdup("Stereo");
  }
  if (type == GNOME_RADIO_CHANNELS_5_1) {
    channels = g_strdup("Surround");
  }
  if (type == GNOME_RADIO_CHANNELS_NONE) {
...
mouse_click_cb (ClutterActor *actor, ClutterButtonEvent *event, ChamplainView *view)
{
	...
	GNOME_RADIO_DEBUG_MSG("Mouse click at: %f %f (%s)\n", lat_s, lon_s, name);
	return TRUE;
}
...
new_station(GSimpleAction *simple, GVariant *parameter, gpointer user_data) {
	GNOME_RADIO_DEBUG_MSG(_("New Internet Radio Station\n"));
  return;
}

static void
search_station(GSimpleAction *simple, GVariant *parameter, gpointer user_data) {
	GNOME_RADIO_DEBUG_MSG(_("Search Internet Radio Station\n"));
  return;
}
...
listen_station(GSimpleAction *simple, GVariant *parameter, gpointer user_data) {
	...
	if (!g_strcmp0(gnome_radio->selected_station_uri, NULL)) {
		gnome_radio_player_new(player, _("http://fm939.wnyc.org/wnycfm"));
	} else {
		gnome_radio_player_new(player, gnome_radio->selected_station_uri);
	}
...
#if 0
	gnome_radio_player_pause(player);
#endif
	return;
}
...
prev_station(GSimpleAction *simple, GVariant *parameter, gpointer user_data) {
	GNOME_RADIO_DEBUG_MSG(_("Previous Internet Radio Station\n"));
	return;
}
...
{
	GNOMERadioStationInfo *stationinfo = NULL;
	/* GList *l = g_list_first(gnome_radio_stations); */
	/* stationinfo = l->data; */

	gnome_radio->selected_station_band =
	    g_strdup(g_object_get_data(G_OBJECT(a), "station_band"));
	GNOME_RADIO_DEBUG_MSG("on_new_station_changed: %s\n",
	    gnome_radio->selected_station_band);
	...
	GNOME_RADIO_DEBUG_MSG("on_new_station_changed: %s\n",
	    gnome_radio->selected_station_uri);
	gnome_radio->selected_station_website =
	...
	/* 		gnome_radio->selected_station_band); */
	gnome_radio_station_update(stationinfo,
						    gnome_radio->selected_station_band,
						    gnome_radio->selected_station_description,
						    gnome_radio->selected_station_name,
						    gnome_radio->selected_station_location,
...
{
	GNOMERadioStationInfo *station = NULL;

	/* if (gnome_radio->selected_station_uri != NULL) */
	/* 	g_free(gnome_radio->selected_station_uri); */

	gnome_radio->selected_station_uri = g_strdup(g_object_get_data(G_OBJECT(a), "station_uri"));

	GNOME_RADIO_DEBUG_MSG("on_station_select_changed: %s\n", gnome_radio->selected_station_uri);
	...
	gnome_radio->selected_station_name = g_strdup(g_object_get_data(G_OBJECT(a), "station_name"));
	/* gnome_radio_history = g_list_add(GLIST(gnome_radio_history), (GNOMERadioStationInfo *)station); */
	/* gnome_radio_helper_main(selected_station_uri); */
}

static void
gnome_radio_window_cb (GtkApplication *app,
		gpointer user_data)
{
			    ...
			    gpointer user_data)
{
	...
	case GTK_RESPONSE_ACCEPT:
		...
		gnome_radio_station_update (stationinfo, selected_station_band, selected_station_description, selected_station_name, selected_station_location, selected_station_uri, selected_station_website);
		break;
	default:
		GNOME_RADIO_DEBUG_MSG (_("Nothing\n\n"));
		break;
	}
		    ...
		    gpointer user_data)
{
	...
	/* 		gnome_radio->selected_station_location); */
	/* gnome_radio_helper_run(selected_station_uri, */
	/* 		gnome_radio->selected_station_name, */
...
/* 	GNOME_RADIO_DEBUG_MSG ("geocode_place_new:town: %s\n", geocode_place_get_town(place)); */
/* 	GNOME_RADIO_DEBUG_MSG ("geocode_place_new:country: %s\n", geocode_place_get_country(place)); */
...
/* 	GNOME_RADIO_DEBUG_MSG ("geocode_place_get_town: %s\n", geocode_place_get_town(place)); */
/* 	GNOME_RADIO_DEBUG_MSG ("geocode_place_get_country: %s\n", geocode_place_get_country(place)); */
...
*/
	...
	player = gst_player_new (NULL, gst_player_g_main_context_signal_dispatcher_new(NULL));
	/* g_object_set_data(G_OBJECT(widget), "station_uri", g_value_get_string(&value)); */
	gnome_radio_player_new(player, g_value_get_string(&value));
...
static void activate(GtkApplication *app, gpointer user_data)
{
	...
	GtkListStore *model;
	GtkTreeIter iter;
	GNOMERadioStationInfo *stationinfo, *localstation;
	guint context_id;
  const char *desc;
	bindtextdomain (GETTEXT_PACKAGE, GNOME_RADIO_LOCALEDIR);
