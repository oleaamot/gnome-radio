...
#include "gtk-internet-radio-locator-markers.h"
...
GtkWidget *window;
GTKInternetRadioLocatorData *gtk_internet_radio_locator;

GtkWidget *input;

GtkEntryCompletion *completion;

GList *gtk_internet_radio_locator_archivers;
GList *gtk_internet_radio_locator_listeners;
GList *gtk_internet_radio_locator_programs;
GList *gtk_internet_radio_locator_stations;
GList *gtk_internet_radio_locator_streams;
...
mouse_click_cb (ClutterActor *actor, ClutterButtonEvent *event, ChamplainView *view)
{
	...
	GTK_INTERNET_RADIO_LOCATOR_DEBUG_MSG("Mouse click at: %f %f (%s)\n", lat, lon, name);
	return TRUE;
}
...
listen_station(GSimpleAction *simple, GVariant *parameter, gpointer user_data) {
	gtk_internet_radio_locator_player_stop(player);
	...
	if (!g_strcmp0(gtk_internet_radio_locator->selected_station_uri, NULL)) {
		gtk_internet_radio_locator_player_new(player, _("http://fm939.wnyc.org/wnycfm"));
	} else {
		gtk_internet_radio_locator_player_new(player, gtk_internet_radio_locator->selected_station_uri);
	}
...
#if 0
  gtk_internet_radio_locator_player_pause(player);
#endif
  return;
}
...
void on_new_station_changed(GtkWidget * a, gpointer user_data)
{
	...
	/* GList *l = g_list_first(gtk_internet_radio_locator_stations); */
	/* stationinfo = l->data; */

	gtk_internet_radio_locator->selected_station_band =
	    g_strdup(g_object_get_data(G_OBJECT(a), "station_band"));
	GTK_INTERNET_RADIO_LOCATOR_DEBUG_MSG("on_new_station_changed: %s\n",
	    gtk_internet_radio_locator->selected_station_band);
	...
	GTK_INTERNET_RADIO_LOCATOR_DEBUG_MSG("on_new_station_changed: %s\n",
	    gtk_internet_radio_locator->selected_station_uri);
	gtk_internet_radio_locator->selected_station_website =
	...
	/* 		gtk_internet_radio_locator->selected_station_band); */
	gtk_internet_radio_locator_station_update(stationinfo,
						    gtk_internet_radio_locator->selected_station_band,
						    gtk_internet_radio_locator->selected_station_description,
						    gtk_internet_radio_locator->selected_station_name,
						    gtk_internet_radio_locator->selected_station_location,
...
void on_stations_selector_changed(GtkWidget * a, gpointer user_data)
{
	...
	/* if (gtk_internet_radio_locator->selected_station_uri != NULL) */
	/* 	g_free(gtk_internet_radio_locator->selected_station_uri); */

	gtk_internet_radio_locator->selected_station_uri = g_strdup(g_object_get_data(G_OBJECT(a), "station_uri"));

	GTK_INTERNET_RADIO_LOCATOR_DEBUG_MSG("on_station_select_changed: %s\n", gtk_internet_radio_locator->selected_station_uri);
	...
	gtk_internet_radio_locator->selected_station_name = g_strdup(g_object_get_data(G_OBJECT(a), "station_name"));
	/* gtk_internet_radio_locator_history = g_list_add(GLIST(gtk_internet_radio_locator_history), (GTKInternetRadioLocatorStationInfo *)station); */
	/* gtk_internet_radio_locator_helper_main(selected_station_uri); */
}

static void
gtk_internet_radio_locator_window_cb (GtkApplication *app,
		gpointer user_data)
{
...
#endif
	gtk_window_set_title (GTK_WINDOW(window), _("Internet Radio Locator for GTK 4"));
	gtk_window_set_default_size (GTK_WINDOW(window), 1440, 720);

	gtk_internet_radio_locator_app = create_gtk_internet_radio_locator_app();
	gtk_widget_show(gtk_internet_radio_locator_app);

	/* stations_selector = create_stations_selector(selected_station_uri, "gtk_internet_radio_locator.xml"); */
			    ...
			    gpointer user_data)
{
	...
	case GTK_RESPONSE_ACCEPT:
		GTK_INTERNET_RADIO_LOCATOR_DEBUG_MSG("Squeak!\n\n");
		selected_station_band = g_strdup(g_object_get_data(G_OBJECT(station), "station_band"));
		GTK_INTERNET_RADIO_LOCATOR_DEBUG_MSG("on_new_station_select_changed: %s\n", selected_station_band);
		...
		gtk_internet_radio_locator_station_update (stationinfo, selected_station_band, selected_station_description, selected_station_name, selected_station_location, selected_station_uri, selected_station_website);
		break;
	default:
		g_print (_("Nothing\n\n"));
		break;
		    ...
		    gpointer user_data)
{
	...
	/* 		gtk_internet_radio_locator->selected_station_location); */
	/* gtk_internet_radio_locator_helper_run(selected_station_uri, */
	/* 		gtk_internet_radio_locator->selected_station_name, */
...
*/
	...
	player = gst_player_new (NULL, gst_player_g_main_context_signal_dispatcher_new(NULL));
	/* g_object_set_data(G_OBJECT(widget), "station_uri", g_value_get_string(&value)); */
	gtk_internet_radio_locator_player_new(player, g_value_get_string(&value));
	gst_player_play(player);
	return FALSE;
}
